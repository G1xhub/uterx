...
- [x] Plugin installation (download from URL, extract, validate)
- [x] Host API: `uterx_ui::draw_text`, `get_size`, `create_overlay`
- [x] Host API: `uterx_io::read`, `write`
- [x] Host API: `uterx_net::http_get`, `tcp_connect`
- [x] Host API: `uterx_fs::read_file`, `write_file` (sandboxed)
- [x] Host API: `uterx_platform::bt_scan`, `keyring_get`
- [x] Permission checking (grant/deny on first use)
...
| 2026-02-21 | Phase 4 | **Plugin Installation Pipeline** | Implemented `PluginManager::install` for local directory, `.zip`, `.tar.gz`/`.tgz`, and HTTP(S) URL sources. Added download + extraction pipeline, recursive `plugin.toml` discovery, manifest/WASM-path validation, and safe copy into `~/.uterx/plugins/<name>`. Existing plugin gets replaced atomically by directory recreation. Updated CLI `plugin add` output to confirm installed plugin name/version. Added plugin-manager unit test for local directory installation. `cargo test -p uterx-plugin` passes; workspace `cargo check` passes (with pre-existing warnings in other crates). |
| 2026-02-22 | Phase 4 | **UI Host API (draw/get_size/overlay)** | Implemented `uterx_ui` host functions in plugin runtime linker: `draw_text`, `get_size`, `create_overlay`. Added permission gating against granted plugin permissions, guest-memory string decoding helper, and runtime UI state tracking (`UiDrawCall`, `UiOverlay`, pane size map, overlay ID allocation). Added accessors on `PluginInstance` for UI state and a runtime permission test. `cargo test -p uterx-plugin` passes (2 tests); workspace `cargo check` passes (with pre-existing warnings in other crates). |
| 2026-02-22 | Phase 4 | **IO Host API (read/write)** | Implemented `uterx_io::write` and `uterx_io::read` host functions in plugin runtime with `Permission::Io` gating. Added guest-memory byte helpers for reading plugin buffers and writing responses back into plugin memory. Runtime now tracks stream input/output buffers and IO write events; `read` consumes buffered input in-order and `write` appends to stream output buffers. Added integration helpers on `PluginInstance` (`set_stream_input`, `append_stream_input`, `stream_output`, `take_stream_output`, `io_write_calls`) and runtime tests for ordered input consumption/output append behavior. `cargo test -p uterx-plugin` passes (4 tests); workspace `cargo check` passes (with pre-existing warnings in other crates). |
| 2026-02-22 | Phase 4 | **Network Host API (http/tcp)** | Implemented `uterx_net::http_get` and `uterx_net::tcp_connect` host functions in plugin runtime with `Permission::Network` gating. `http_get` reads URL from guest memory, performs blocking request, writes bounded response bytes into guest memory, returns HTTP status code, and records call metadata. `tcp_connect` resolves host/port, establishes a timed TCP connection, stores socket handles in runtime state, and returns a socket ID (or 0 on failure). Added runtime net state/accessors and a local-listener connectivity test for timeout-based TCP connect helper. `cargo test -p uterx-plugin` passes (5 tests); workspace `cargo check` passes (with pre-existing warnings in other crates). |
| 2026-02-22 | Phase 4 | **Filesystem Host API (sandboxed read/write)** | Implemented `uterx_fs::read_file` and `uterx_fs::write_file` host functions with `Permission::Filesystem` gating and strict sandbox path resolution. Added per-plugin filesystem root handling in runtime state and safeguards against absolute paths / parent traversal (`..`). `read_file` returns bounded bytes from sandboxed files into guest memory; `write_file` creates parent directories and writes only inside sandbox root. Added runtime helpers to configure/access sandbox root and tests for traversal rejection + in-root resolution. `cargo test -p uterx-plugin` passes (7 tests); workspace `cargo check` passes (with pre-existing warnings in other crates). |
| 2026-02-22 | Phase 4 | **Platform Host API (bt/keyring)** | Implemented `uterx_platform::bt_scan` and `uterx_platform::keyring_get` host functions in plugin runtime with `Permission::Platform` gating. Added runtime platform mock state for Bluetooth device inventory and keyring values, plus `PluginInstance` accessors to set/read mock devices and keyring entries. `bt_scan` returns discovered-device count; `keyring_get` reads key from guest memory and writes bounded value bytes back to guest memory. Added runtime test coverage for platform state behavior. `cargo test -p uterx-plugin` passes (8 tests); workspace `cargo check` passes (with pre-existing warnings in other crates). |
